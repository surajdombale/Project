...
/// used to show image  
      const imageShow=(id)=>{
        return `http://localhost:8080/category/download/${id}`; 
      }
      ...
      const handleEnable=async(id)=>{
        const rest= await axios.post(`http://localhost:8080/category/enabled/${id}`)
        console.log(rest)
        let enable='desabled';
        if(rest.data){
          enable='enabled'
      ...
      const rest= await axios.delete(`http://localhost:8080/category/${id}`)
...
useEffect(()=>{
  setAxiFilter(axi.filter((item) => search === '' ||  item.name.toLowerCase().includes(search.toLowerCase())).filter((item) => filters.category === 'All' || item.parentName === filters.category));
      ...
      try{
      const respons=  await axios.get("http://localhost:8080/category/getsub");
