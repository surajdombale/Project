...
const EditData = async(id,isChecked,aliasName ,name ,parent ,selectedFile) => {
  ...
  try {
    const respons= await axios.put('http://localhost:8080/ShopmeAdmin/category', formData,{ 
    headers: {
...
// Make a POST request to your Java server with the file data
try {
  const respons= await axios.put('http://localhost:8080/ShopmeAdmin/category/editwithoutphoto', formData1,{ 
headers: {
'Content-Type': 'multipart/form-data',
},
});
